import React, { memo } from "react";

import { Form, Formik } from "formik";
import { MUIDataTableColumnDef, MUIDataTableOptions } from "mui-datatables";
import { array, object, string } from "yup";

import LoadingContainer from "../LoadingContainer";

import { DataTable, DataTableRefreshActionButtonIcon, TextFieldFormDataTable } from "@/components";
import { FieldWrapper } from "@/forms/style";
import { IPeerEvaluationStudent } from "@/transformers/students";
import { ObjectArray, objectToArrayOfObject } from "@/utils/form";

interface IPeerEvaluationStudentsDataTable {
  data: [IPeerEvaluationStudent] | [];
  onRefreshStudents: () => Promise<void>;
}

const PeerEvaluationStudentsDataTable = ({ data, onRefreshStudents }: IPeerEvaluationStudentsDataTable) => {
  const validationSchema = object({
    peerEvaluationStudentTeamNames: array().of(
      object().shape({
        peerEvaluationStudentTeamName: string().required("Team name is required"),
      })
    ),
  });

  const dataTableColumns: MUIDataTableColumnDef[] = [
    {
      name: "id",
      label: "ID",
      options: {
        filter: false,
        display: false,
      },
    },
    {
      name: "userName",
      label: "Student Name",
    },
    {
      name: "userEmail",
      label: "Student Email",
      options: {
        filterType: "textField",
      },
    },
    {
      name: "userEmailVerified",
      label: "Student Verified",
      options: {
        display: false,
        filter: false,
      },
    },
    {
      name: "updatedAt",
      label: "Updated",
      options: {
        filter: true,
        display: false,
      },
    },
    {
      name: "averageCriteriaScore",
      label: "Avg Criteria Score",
      options: {
        display: false,
      },
    },
    {
      name: "averageCriteriaScoreByTeamMember",
      label: "Avg Criteria Score by Team Member",
      options: {
        display: false,
      },
    },
    {
      name: "systemCalculatedMark",
      label: "System Calculated Mark",
      options: {
        display: false,
      },
    },
    {
      name: "systemAdjustedMark",
      label: "System Adjusted Mark",
      options: {
        display: false,
      },
    },
    {
      name: "lecturerAdjustedMark",
      label: "Lectured Adjusted Mark",
      options: {
        display: false,
      },
    },
    {
      name: "mark",
      label: "Mark",
    },
    {
      name: "peerEvaluationStudentTeamName",
      label: "Team Name",
      options: {
        display: true,
        customBodyRender: (_, tableMeta, updateValue) => (
          <FieldWrapper marginBottom="3em">
            <TextFieldFormDataTable
              updateDataTableFormValue={updateValue}
              validationSchema={validationSchema}
              validationFieldPath={"peerEvaluationStudentTeamNames.peerEvaluationStudentTeamName"}
              testId="text-test-id"
              name={`peerEvaluationStudentTeamNames[${tableMeta.rowIndex}].peerEvaluationStudentTeamName`}
              props={{
                name: `peerEvaluationStudentTeamNames[${tableMeta.rowIndex}].peerEvaluationStudentTeamName`,
                required: true,
                fullWidth: true,
                label: "Team name",
                type: "text",
                variant: "outlined",
              }}
            />
          </FieldWrapper>
        ),
      },
    },
    {
      name: "peerEvaluationStudentTeamMark",
      label: "Team Mark",
    },
    {
      name: "peerEvaluationReviewedIsCompleted",
      label: "Peer Evaluation Completed",
    },
  ];

  const dataTableOptions: MUIDataTableOptions = {
    textLabels: {
      body: {
        noMatch: "Sorry, no students",
      },
    },
    responsive: "simple",
    tableBodyMaxHeight: "100%",
    selectableRows: "single",
    selectableRowsHeader: true,
    rowHover: true,
    download: true,
    sort: false,
    filter: true,
    search: true,
    viewColumns: true,
    pagination: false,
    print: false,
    enableNestedDataAccess: ".",
    downloadOptions: {
      filename: `peer_evaluations_students-${new Date().getTime()}.csv`,
    },
    draggableColumns: {
      enabled: true,
    },
    rowsPerPage: 100,
    customToolbar: (_) => (
      <DataTableRefreshActionButtonIcon
        onClick={onRefreshStudents}
        testId={"peer-evaluation-students-refresh-icon"}
        toolTipLabel={"Refresh"}
      />
    ),
  };

  if (data.length === 0) {
    return <LoadingContainer loading />;
  }

  return (
    <Formik
      initialValues={{
        peerEvaluationStudentTeamNames: objectToArrayOfObject(
          "peerEvaluationStudentTeamName",
          data as unknown as ObjectArray
        ),
      }}
      validationSchema={validationSchema}
      onSubmit={(values) => console.log(values)}
    >
      {() => (
        <Form>
          <DataTable
            testId={"peer-evaluation-students-datatable"}
            isVisible={!!data}
            data={data}
            columns={dataTableColumns}
            options={dataTableOptions}
          />
          <button type="submit">Submit</button>
        </Form>
      )}
    </Formik>
  );
};

export default memo(PeerEvaluationStudentsDataTable);
